...
DATA_PATH = 'examples/data/math_test.json'
...
def generate_answer_for_test(data_path):
	...
	header = ["id", "question", "choices", "answer"]
	first = True             
	start_index = 0
	...
	for i, chunk in enumerate(tqdm(chunks)):
		...
		with open("examples/data/math_public_test_result_v2.csv", "a", encoding="utf-8") as f:
			writer = csv.writer(f)
			if first:
				writer.writerow(header)
				first = False
			for data, result in zip(json_chunks[i], results):
					writer.writerow([data["id"], data["question"], data["choices"], result.assistant])

if __name__ == '__main__':
	generate_answer_for_test(DATA_PATH)